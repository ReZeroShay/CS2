...
auto loadBigFont() {
    ...
    NotificationFont1 =
        ImGui::GetIO().Fonts->AddFontFromFileTTF("assets/fonts/JetBrainsMonoNerdFont-Regular.ttf",
                                                 18.f * 2 * ImGui::GetStyle().FontScaleDpi, nullptr, ranges.Data);
    ImFontConfig config;
    config.MergeMode = true; // 关键：设置为合并模式,将中文和表情字体合并
    config.PixelSnapH = true;
    // 增强字体粗细 / 对比度
    ...
    NotificationFont2 = ImGui::GetIO().Fonts->AddFontFromFileTTF(
        R"(C:\Windows\Fonts\msyh.ttc)", 18.0f * 2 * ImGui::GetStyle().FontScaleDpi, &config,
...
auto createOverlayHWND() -> HWND {
    ...
    HWND hwnd = ::CreateWindowExW(WS_EX_TOPMOST | WS_EX_LAYERED | WS_EX_TOOLWINDOW, wc.lpszClassName, L"classui04",
                                  WS_POPUP, 0, 0, screenWidth, screenHeight, nullptr, nullptr, wc.hInstance, nullptr);
    return hwnd;
}
...
int overlay(std::optional<RequestContext> request, HWND hwnd) {
    ...
    */
    ...
    request->overlay(hwnd);

    cheat::SnapshotManager snapshots;
    // auto reader = std::make_unique<MemoryReaderAdapter<RequestContext>>(pid);

    std::jthread jt(cheat::readTask, std::ref(snapshots),
                    MemoryReaderAdapter<RequestContext>(std::move(request.value())));
    ...
    while (!done) {
        ...
        if (!overlayContext.isMenuVisible && g_SwapChainOccluded &&
